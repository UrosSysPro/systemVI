	...
	switch(event){
		...
		case ESP_GAP_BLE_SEC_REQ_EVT:
			for(int i=0;i<ESP_BD_ADDR_LEN;i++){
				printf("%x",param->ble_security.ble_req.bd_addr[i]);
			}
			printf("\n");
		...
		case ESP_GAP_BLE_AUTH_CMPL_EVT:
			...
			printf("remote BD_ADDR: %08x %04x",
				(bd_addr[0]<<24)+(bd_addr[1]<<16)+(bd_addr[2]<< 8)+(bd_addr[3]<< 0),
				(bd_addr[4]<< 8)+(bd_addr[5]<< 0)
			);
			printf("addres type %d",param->ble_security.auth_cmpl.addr_type);
...
void hidd_callback(esp_hidd_cb_event_t event, esp_hidd_cb_param_t *param){
	switch(event){
		...
		case ESP_HIDD_EVENT_BLE_CONNECT:{
			printf("esp hidd event ble connect\n");
			connection_id=param->connect.conn_id;
		}break;
		case ESP_HIDD_EVENT_BLE_DISCONNECT:{
			connected=false;
			printf("esp hidd event ble disconnect");
		}break;
		case ESP_HIDD_EVENT_BLE_VENDOR_REPORT_WRITE_EVT:{
			printf("esp hidd event ble vendor report write event");
		}break;
		case ESP_HIDD_EVENT_BLE_LED_REPORT_WRITE_EVT:{
			printf("esp hidd event ble led report write event");
		}break;
		default:{} break;
	}
}
