...
def hearth(center:Point = Point(0,0),count:Int = 400, scale:Double = 10):List[Point] = {
  (for(i<-0 to count)yield{
    val t = i.toDouble/count*Math.PI*2
    val x = (16 * Math.pow(Math.sin(t), 3)) * scale - center.x
    val y = -(13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t)) * scale - center.y
    Point(x,y)
  }).toList
}
