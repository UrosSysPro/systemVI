...
object ThreeDPrintedProductService{
  private val printingCostPerHour = 50
  ...
  case class ThreeDPrintedProduct(
    uuid: UUID,
    name: String,
    components: List[ThreeDPrintedProductComponent]
  ){
    def totalPrice(): Int = {
  ...
  val seaferersGame56Players = ThreeDPrintedProduct(
    UUID(3,3),
    "Catan: Base Game + 5/6 Player Expansion + Seaferers Expansion + 5/6 Players Seaferers Expansion",
    List(
      // Tiles
  ...
  def getAll(): EventStream[List[ThreeDPrintedProduct]] = {
    EventStream.fromValue(games);
  }

  def get(productUUID: UUID): EventStream[Option[ThreeDPrintedProduct]] = {
    EventStream.fromValue(games.find(_.uuid==productUUID))
  }
}
