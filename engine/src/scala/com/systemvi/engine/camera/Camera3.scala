...
import com.systemvi.engine.camera.ProjectionType.{Orthographic, Perspective}
import org.joml.{Matrix4f, Vector2f, Vector3f}

enum ProjectionType {
  case Perspective(fov:Float, aspect:Float, near:Float, far:Float) extends ProjectionType
  case Orthographic(left:Float, right:Float, bottom:Float, top:Float, near:Float, far:Float) extends ProjectionType
}
...
class Camera3(val position: Vector3f, val rotation: Vector3f, val scale: Vector3f) {
  ...
  var projectionType: ProjectionType = null
  ...
  def perspective(near: Float, far: Float, aspect: Float, fov: Float): Camera3 = {
    projectionType=Perspective(fov,aspect, near, far)
